...
`
	...
	for index, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// TensorGrid fork: a submission that fails immediately is refunded
			// rather than settled at zero (see
			// TestExecuteTaskSubmissionRefundsImmediateFailureInsteadOfSettling).
			// BillingSession.Refund returns funds asynchronously and writes no
			// consume log.
			refunded := tc.status == "FAILURE"
			var updated model.User
			if refunded {
				require.Eventually(t, func() bool {
					return db.First(&updated, user.Id).Error == nil && updated.Quota == initial
