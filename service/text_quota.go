...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.ImageTokens != 0 {
		...
		// TensorGrid usage reporting reads explicit *_tokens keys; image_output
		// alone is ambiguous about direction.
		other.SetPublic("image_input_tokens", summary.ImageTokens)
	}
	if summary.ImageOutputTokens != 0 {
		other.SetPublic("image_output_tokens", summary.ImageOutputTokens)
	}
	...
	if summary.AudioInputPrice > 0 && summary.AudioTokens > 0 {
		...
		other.SetPublic("audio_input_tokens", summary.AudioTokens)
		other.SetPublic("audio_input_price", summary.AudioInputPrice)
	}
	if summary.AudioOutputTokens != 0 {
		other.SetPublic("audio_output_tokens", summary.AudioOutputTokens)
	}
