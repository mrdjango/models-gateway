...
// An image reservation tops up an earlier wallet pre-consume through
// WalletFunding.PreConsume. The refund of a failed request must return both
// parts; PreConsume used to overwrite consumed with the increment alone.
func TestBillingSessionImageReserveRefundReturnsWholeReservation(t *testing.T) {
	truncate(t)

	const userID = 703
	seedUser(t, userID, 500_000)
	require.NoError(t, model.DecreaseUserQuota(userID, 50_000, false))

	funding := &WalletFunding{userId: userID, consumed: 50_000}
	session := &BillingSession{
		relayInfo:        &relaycommon.RelayInfo{UserId: userID, IsPlayground: true, ImageRequestCount: 2},
		funding:          funding,
		preConsumedQuota: 50_000,
	}
...
// Settling a session that was already refunded must not return the
// pre-consume a second time.
func TestBillingSessionSettleAfterRefundMovesNoQuota(t *testing.T) {
	truncate(t)
