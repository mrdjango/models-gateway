...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	otherStr := EnrichTensorGridLogOther(userId, other).JSONString()
	// 判断是否需要记录 IP
	needRecordIp := false
...
type RecordConsumeLogParams struct {
	...
	// BillingRequestId overrides the log's own request id when a TensorGrid
	// credit event must be keyed to a different billing request (for example a
	// violation fee charged alongside the request it belongs to).
