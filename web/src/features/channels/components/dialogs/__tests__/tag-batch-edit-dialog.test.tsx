...
import { TagBatchEditDialog } from '../tag-batch-edit-dialog'

/**
 * Regression coverage for the tag batch-edit dialog silently overwriting
 * every channel in a tag with one arbitrary model. Since the 2026-09-17
 * per-model channel split, channels sharing a tag normally each carry a
 * single, different model, so the dialog must only send `models` in the
 * save request when the operator explicitly typed a value in this session.
 */
...
function renderDialog(tag: string): ReturnType<typeof render> {
  return render(
    <QueryClientProvider client={createQueryClient()}>
      <ChannelsProvider>
        <WithTag tag={tag}>
          <TagBatchEditDialog open onOpenChange={() => undefined} />
        </WithTag>
      </ChannelsProvider>
    </QueryClientProvider>
  )
...
test('typing a models value applies it as a tag-wide override', async () => {
  const user = userEvent.setup()
  renderDialog('prod-pool')
...
test('starting from current models seeds the textarea and applies it on save', async () => {
  const user = userEvent.setup()
  renderDialog('prod-pool')
  ...
  ).toHaveValue('claude-3-opus,gpt-4o')
  await user.click(screen.getByRole('button', { name: 'Save Changes' }))
